...
;;; Commentary:
;;
;; This initializes Emacs Application Framework (EAF)
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;;; Code:
...
:if my-cap/eaf-ready-p ; Use the constant defined in init-const.el
:custom
(eaf-start-python-process-when-require t) ; Ensure python process starts when needed
...
:demand ; Load EAF itself eagerly if the environment is ready
:bind
(("M-z r" . eaf-open-rss-reader)
("M-m r" . eaf-open-rss-reader)
("M-#" . eaf-open-pyqterminal))
:config
(when (display-graphic-p) ; Only load graphical EAF apps if in a graphical environment
...
;; PopwebPac (Integrated with EAF)
(use-package popweb
:if my-cap/eaf-ready-p ; Conditional on EAF readiness
