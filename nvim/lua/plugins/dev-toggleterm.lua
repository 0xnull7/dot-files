            ...
            -- NEW: Keymap for PdfLaTeX: <leader>rlp
            {
                ...
                function()
                    ...
                    if ft == "tex" then
                        ...
                        local cmd = create_run_cmd(
                            file,
                            [[cd %d && latexmk -pdf -pdflatex -interaction=nonstopmode -synctex=1 -halt-on-error -output-directory=build -e "$pdflatex = 'pdflatex %O -shell-escape %S'" %f]]
                        )
            ...
            -- NEW: Keymap for XeLaTeX: <leader>rlx
            {
                ...
                function()
                    ...
                    if ft == "tex" then
                        ...
                        local cmd = create_run_cmd(
                            file,
                            [[cd %d && latexmk -pdfxe -xelatex -interaction=nonstopmode -synctex=1 -halt-on-error -output-directory=build -e "$xelatex = 'xelatex %O -shell-escape %S'" %f]]
                        )
